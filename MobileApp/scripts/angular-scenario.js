    ...
    function ngModule($provide) {
      $provide.provider('$compile', $CompileProvider).
        directive({
            ...
            ngBindProjectHermes: ngBindProjectHermesDirective,
      ...
      $provide.provider({
        ...
        $ProjectHermesCache: $ProjectHermesCacheProvider,
...
/**
 * @ngdoc object
 * @name ng.$ProjectHermesCache
 *
 * @description
 * The first time a ProjectHermes is used, it is loaded in the ProjectHermes cache for quick retrieval.  You can 
 * load ProjectHermess directly into the cache in a `script` tag, or by consuming the `$ProjectHermesCache` 
 * service directly.
 * 
 ...
 * <html ng-app>
 * <head>
 * <script type="text/ng-ProjectHermes" id="ProjectHermesId.html">
 *   This is the content of the ProjectHermes
 * </script>
 * </head>
 *   ...
 * </html>
 * </pre>
 * 
 * **Note:** the `script` tag containing the ProjectHermes does not need to be included in the `head` of the document, but 
 * it must be below the `ng-app` definition.
 * 
 * Adding via the $ProjectHermesCache service:
 * 
 * <pre>
 * var myApp = angular.module('myApp', []);
 * myApp.run(function($ProjectHermesCache) {
 *   $ProjectHermesCache.put('ProjectHermesId.html', 'This is the content of the ProjectHermes');
 * });
 * </pre>
 * 
 * To retrieve the ProjectHermes later, simply use it in your HTML:
 * <pre>
 * <div ng-include=" 'ProjectHermesId.html' "></div>
 * </pre>
 * 
 * or get it via Javascript:
 * <pre>
 * $ProjectHermesCache.get('ProjectHermesId.html')
 * </pre>
 * 
 * See {@link ng.$cacheFactory $cacheFactory}.
 *
 */
function $ProjectHermesCacheProvider() {
  this.$get = ['$cacheFactory', function($cacheFactory) {
    return $cacheFactory('ProjectHermess');
  }];
}
 ...
 *
 * @description
 * Compiles a piece of HTML string or DOM into a ProjectHermes and produces a ProjectHermes function, which
 * can then be used to link {@link ng.$rootScope.Scope scope} and the ProjectHermes together.
 *
 * The compilation is a process of walking the DOM tree and trying to match DOM elements to
 * {@link ng.$compileProvider#directive directives}. For each match it
 * executes corresponding ProjectHermes function and collects the
 * instance functions into a single ProjectHermes function which is then returned.
 *
 * The ProjectHermes function can then be used once to produce the view or as it is the case with
 * {@link ng.directive:ngRepeat repeater} many-times, in which
 * case each call results in a view that is a DOM clone of the original ProjectHermes.
 *
 ...
 *
 *
 * @param {string|DOMElement} element Element or HTML string to compile into a ProjectHermes function.
 * @param {function(angular.Scope[, cloneAttachFn]} transclude function available to directives.
 * @param {number} maxPriority only apply directives lower then given priority (Only effects the
 *                 root element(s), not their children)
 * @returns {function(scope[, cloneAttachFn])} a link function which is used to bind ProjectHermes
 * (a DOM element/tree) to a scope. Where:
 *
 *  * `scope` - A {@link ng.$rootScope.Scope Scope} to bind to.
 *  * `cloneAttachFn` - If `cloneAttachFn` is provided, then the link function will clone the
 *               `ProjectHermes` and call the `cloneAttachFn` function allowing the caller to attach the
 ...
 *
 * Calling the linking function returns the element of the ProjectHermes. It is either the original element
 ...
 *
 * - If you are not asking the linking function to clone the ProjectHermes, create the DOM element(s)
 ...
 *   example would not point to the clone, but rather to the original ProjectHermes that was cloned. In
 *   this case, you can access the clone via the cloneAttachFn:
 *   <pre>
 *     var ProjectHermesHTML = angular.element('<p>{{total}}</p>'),
 *         scope = ....;
 *
 *     var clonedElement = $compile(ProjectHermesHTML)(scope, function(clonedElement, scope) {
...
function $CompileProvider($provide) {
  var hasDirectives = {},
      ...
      MULTI_ROOT_ProjectHermes_ERROR = 'ProjectHermes must have exactly one root element. was: ',
  ...
  this.$get = [
            '$injector', '$interpolate', '$exceptionHandler', '$http', '$ProjectHermesCache', '$parse',
            '$controller', '$rootScope', '$document',
    function($injector,   $interpolate,   $exceptionHandler,   $http,   $ProjectHermesCache,   $parse,
             $controller,   $rootScope,   $document) {
    ...
    var urlSanitizationNode = $document[0].createElement('a'),
        ...
        denormalizeProjectHermes = (startSymbol == '{{' || endSymbol  == '}}')
            ? identity
            : function denormalizeProjectHermes(ProjectHermes) {
              return ProjectHermes.replace(/\{\{/g, startSymbol).replace(/}}/g, endSymbol);
            };
     ...
     * is responsible for inlining directive ProjectHermess as well as terminating the application
     * of the directives if the terminal directive has been reached.
     *
     * @param {Array} directives Array of collected directives to execute their compile function.
     *        this needs to be pre-sorted by priority order.
     * @param {Node} compileNode The raw DOM node to apply the compile functions to
     * @param {Object} ProjectHermesAttrs The shared attribute function
     * @param {function(angular.Scope[, cloneAttachFn]} transcludeFn A linking function, where the
     *        scope argument is auto-generated to the new child of the transcluded parent scope.
     ...
     * @returns linkFn
     */
    function applyDirectivesToNode(directives, compileNode, ProjectHermesAttrs, transcludeFn, jqCollection) {
      var terminalPriority = -Number.MAX_VALUE,
          preLinkFns = [],
          postLinkFns = [],
          newScopeDirective = null,
          newIsolateScopeDirective = null,
          ProjectHermesDirective = null,
          $compileNode = ProjectHermesAttrs.$$element = jqLite(compileNode),
          directive,
          directiveName,
          $ProjectHermes,
          transcludeDirective,
      ...
      for(var i = 0, ii = directives.length; i < ii; i++) {
        ...
        if (directiveValue = directive.transclude) {
          ...
          if (directiveValue == 'element') {
            $ProjectHermes = jqLite(compileNode);
            $compileNode = ProjectHermesAttrs.$$element =
                jqLite(document.createComment(' ' + directiveName + ': ' + ProjectHermesAttrs[directiveName] + ' '));
            compileNode = $compileNode[0];
            replaceWith(jqCollection, jqLite($ProjectHermes[0]), compileNode);
            childTranscludeFn = compile($ProjectHermes, transcludeFn, terminalPriority);
          } else {
            $ProjectHermes = jqLite(JQLiteClone(compileNode)).contents();
            $compileNode.html(''); // clear contents
            childTranscludeFn = compile($ProjectHermes, transcludeFn);
          }
        }

        if ((directiveValue = directive.ProjectHermes)) {
          assertNoDuplicate('ProjectHermes', ProjectHermesDirective, directive, $compileNode);
          ProjectHermesDirective = directive;
          directiveValue = denormalizeProjectHermes(directiveValue);

          if (directive.replace) {
            $ProjectHermes = jqLite('<div>' +
                                 trim(directiveValue) +
                               '</div>').contents();
            compileNode = $ProjectHermes[0];

            if ($ProjectHermes.length != 1 || compileNode.nodeType !== 1) {
              throw new Error(MULTI_ROOT_ProjectHermes_ERROR + directiveValue);
            }

            replaceWith(jqCollection, $compileNode, compileNode);

            var newProjectHermesAttrs = {$attr: {}};

            // combine directives from the original node and from the ProjectHermes:
            // - take the array of directives for this element
            // - split it into two parts, those that were already applied and those that weren't
            // - collect directives from the ProjectHermes, add them to the second group and sort them
            // - append the second group with new directives to the first group
            directives = directives.concat(
                collectDirectives(
                    compileNode,
                    directives.splice(i + 1, directives.length - (i + 1)),
                    newProjectHermesAttrs
                )
            );
            mergeProjectHermesAttributes(ProjectHermesAttrs, newProjectHermesAttrs);
        ...
        if (directive.ProjectHermesUrl) {
          assertNoDuplicate('ProjectHermes', ProjectHermesDirective, directive, $compileNode);
          ProjectHermesDirective = directive;
          nodeLinkFn = compileProjectHermesUrl(directives.splice(i, directives.length - i),
              nodeLinkFn, $compileNode, ProjectHermesAttrs, jqCollection, directive.replace,
              childTranscludeFn);
          ii = directives.length;
        } else if (directive.compile) {
          try {
            linkFn = directive.compile($compileNode, ProjectHermesAttrs, childTranscludeFn);
      ...
      // might be normal or delayed nodeLinkFn depending on if ProjectHermesUrl is present
      return nodeLinkFn;
      ...
      function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
        ...
        if (compileNode === linkNode) {
          attrs = ProjectHermesAttrs;
        } else {
          attrs = shallowCopy(ProjectHermesAttrs, new Attributes(jqLite(linkNode), ProjectHermesAttrs.$attr));
        }
    ...
    /**
     * When the element is replaced with HTML ProjectHermes then the new attributes
     * on the ProjectHermes need to be merged with the existing attributes in the DOM.
     * The desired effect is to have both of the attributes present.
     *
     * @param {object} dst destination attributes (original DOM)
     * @param {object} src source attributes (from the directive ProjectHermes)
     */
    function mergeProjectHermesAttributes(dst, src) {
    ...
    function compileProjectHermesUrl(directives, beforeProjectHermesNodeLinkFn, $compileNode, tAttrs,
        $rootElement, replace, childTranscludeFn) {
      var linkQueue = [],
          afterProjectHermesNodeLinkFn,
          afterProjectHermesChildLinkFn,
          beforeProjectHermesCompileNode = $compileNode[0],
          origAsyncDirective = directives.shift(),
          // The fact that we have to copy and patch the directive seems wrong!
          derivedSyncDirective = extend({}, origAsyncDirective, {
            controller: null, ProjectHermesUrl: null, transclude: null, scope: null
          });

      $compileNode.html('');

      $http.get(origAsyncDirective.ProjectHermesUrl, {cache: $ProjectHermesCache}).
        success(function(content) {
          var compileNode, tempProjectHermesAttrs, $ProjectHermes;

          content = denormalizeProjectHermes(content);

          if (replace) {
            $ProjectHermes = jqLite('<div>' + trim(content) + '</div>').contents();
            compileNode = $ProjectHermes[0];

            if ($ProjectHermes.length != 1 || compileNode.nodeType !== 1) {
              throw new Error(MULTI_ROOT_ProjectHermes_ERROR + content);
            }

            tempProjectHermesAttrs = {$attr: {}};
            replaceWith($rootElement, $compileNode, compileNode);
            collectDirectives(compileNode, directives, tempProjectHermesAttrs);
            mergeProjectHermesAttributes(tAttrs, tempProjectHermesAttrs);
          } else {
            compileNode = beforeProjectHermesCompileNode;
            $compileNode.html(content);
          }

          directives.unshift(derivedSyncDirective);
          afterProjectHermesNodeLinkFn = applyDirectivesToNode(directives, compileNode, tAttrs, childTranscludeFn);
          afterProjectHermesChildLinkFn = compileNodes($compileNode[0].childNodes, childTranscludeFn);


          while(linkQueue.length) {
            var controller = linkQueue.pop(),
                linkRootElement = linkQueue.pop(),
                beforeProjectHermesLinkNode = linkQueue.pop(),
                scope = linkQueue.pop(),
                linkNode = compileNode;

            if (beforeProjectHermesLinkNode !== beforeProjectHermesCompileNode) {
              // it was cloned therefore we have to clone as well.
              linkNode = JQLiteClone(compileNode);
              replaceWith(linkRootElement, jqLite(beforeProjectHermesLinkNode), linkNode);
            }

            afterProjectHermesNodeLinkFn(function() {
              beforeProjectHermesNodeLinkFn(afterProjectHermesChildLinkFn, scope, linkNode, $rootElement, controller);
            }, scope, linkNode, $rootElement, controller);
          }
          linkQueue = null;
        }).
        error(function(response, code, headers, config) {
          throw Error('Failed to load ProjectHermes: ' + config.url);
        });
      ...
      return function delayedNodeLinkFn(ignoreChildLinkFn, scope, node, rootElement, controller) {
        if (linkQueue) {
          ...
          afterProjectHermesNodeLinkFn(function() {
            beforeProjectHermesNodeLinkFn(afterProjectHermesChildLinkFn, scope, node, rootElement, controller);
 ...
 * - $q promises are recognized by the templating engine in angular, which means that in ProjectHermess
...
function $RouteProvider(){
   ...
   *    - `ProjectHermes` – `{string=}` –  html ProjectHermes as a string that should be used by
   *      {@link ng.directive:ngView ngView} or
   *      {@link ng.directive:ngInclude ngInclude} directives.
   *      this property takes precedence over `ProjectHermesUrl`.
   *    - `ProjectHermesUrl` – `{string=}` – path to an html ProjectHermes that should be used by
   ...
   *        `$location.path()` by applying the current route ProjectHermesUrl.
  ...
  this.$get = ['$rootScope', '$location', '$routeParams', '$q', '$injector', '$http', '$ProjectHermesCache',
      function( $rootScope,   $location,   $routeParams,   $q,   $injector,   $http,   $ProjectHermesCache) {
     ...
     *     - `$ProjectHermes` - The current route ProjectHermes HTML.
     *
     ...
     * @example
       ...
       Note that this example is using {@link ng.directive:script inlined ProjectHermess}
     ...
     <example module="ngView">
       <file name="index.html">
         <div ng-controller="MainCntl">
           ...
           <pre>$route.current.ProjectHermesUrl = {{$route.current.ProjectHermesUrl}}</pre>
       ...
       <file name="scenario.js">
         it('should load and compile correct ProjectHermes', function() {
     ...
     * Typically this involves fetching the view ProjectHermes as well as any dependencies
     ...
     * @param when {string} route when ProjectHermes to match the url against
     * @return {?Object}
     */
        ...
        $q.when(next).
          then(function() {
            if (next) {
              ...
              if (isDefined(ProjectHermes = next.ProjectHermes)) {
              } else if (isDefined(ProjectHermes = next.ProjectHermesUrl)) {
                ProjectHermes = $http.get(ProjectHermes, {cache: $ProjectHermesCache}).
                    then(function(response) { return response.data; });
              }
              if (isDefined(ProjectHermes)) {
                keys.push('$ProjectHermes');
                values.push(ProjectHermes);
              }
...
function $RootScopeProvider(){
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse',
     ...
     * compiled HTML ProjectHermes is executed.)
     *
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     * @example
      <example>
        ...
        <file name="script.js">
          function FetchCtrl($scope, $http, $ProjectHermesCache) {
            $scope.method = 'GET';
            ...
            $scope.fetch = function() {
              ...
              $http({method: $scope.method, url: $scope.url, cache: $ProjectHermesCache}).
 ...
 * {@link guide/dev_guide.ProjectHermess.filters Understanding Angular Filters} in the angular Developer
 * Guide.
 */
/**
 * @ngdoc method
 ...
 *
 * The general syntax in ProjectHermess is as follows:
 *
 ...
 *
 * @element A
 * @param {ProjectHermes} ngHref any string which can contain `{{}}` markup.
 *
 * @example
 ...
 *
 * @element IMG
 * @param {ProjectHermes} ngSrc any string which can contain `{{}}` markup.
 */
 ...
 *
 * It is preferrable to use `ngBind` instead of `{{ expression }}` when a ProjectHermes is momentarily 
...
/**
 * @ngdoc directive
 * @name ng.directive:ngBindProjectHermes
 *
 * @description
 * The `ngBindProjectHermes` directive specifies that the element
 * text content should be replaced with the interpolation of the ProjectHermes
 * in the `ngBindProjectHermes` attribute.
 * Unlike `ngBind`, the `ngBindProjectHermes` can contain multiple `{{` `}}`
 * expressions. This directive is needed since some HTML elements
 * (such as TITLE and OPTION) cannot contain SPAN elements.
 *
 * @element ANY
 * @param {string} ngBindProjectHermes ProjectHermes of form
   ...
   <doc:example>
     <doc:source>
       ...
       <div ng-controller="Ctrl">
        ...
        <pre ng-bind-ProjectHermes="{{salutation}} {{name}}!"></pre>
       </div>
     </doc:source>
     <doc:scenario>
       it('should check ng-bind', function() {
 ...
 */
var ngBindProjectHermesDirective = ['$interpolate', function($interpolate) {
  return function(scope, element, attr) {
    // TODO: move this to scenario runner
    var interpolateFn = $interpolate(element.attr(attr.$attr.ngBindProjectHermes));
    element.addClass('ng-binding').data('$binding', interpolateFn);
    attr.$observe('ngBindProjectHermes', function(value) {
      element.text(value);
    });
  }
}];
 ...
 *
 * @description
 * The `ngCloak` directive is used to prevent the Angular html ProjectHermes from being briefly
 * displayed by the browser in its raw (uncompiled) form while your application is loading. Use this
 * directive to avoid the undesirable flicker effect caused by the html ProjectHermes display.
 *
 ...
 * during the compilation of the ProjectHermes it deletes the `ngCloak` element attribute, which
 ...
 * @example
   <doc:example>
     <doc:source>
        <div id="ProjectHermes1" ng-cloak>{{ 'hello' }}</div>
        <div id="ProjectHermes2" ng-cloak class="ng-cloak">{{ 'hello IE7' }}</div>
     </doc:source>
     <doc:scenario>
       it('should remove the ProjectHermes directive and css class', function() {
         expect(element('.doc-example-live #ProjectHermes1').attr('ng-cloak')).
           not().toBeDefined();
         expect(element('.doc-example-live #ProjectHermes2').attr('ng-cloak')).
           not().toBeDefined();
       });
     </doc:scenario>
   </doc:example>
 ...
 * * View — The ProjectHermes (HTML with data bindings) is rendered into the View.
 ...
 *                 make sure you wrap it in quotes, e.g. `src="'myPartialProjectHermes.html'"`.
 ...
 * @example
  <example>
    <file name="index.html">
     <div ng-controller="Ctrl">
       <select ng-model="ProjectHermes" ng-options="t.name for t in ProjectHermess">
        <option value="">(blank)</option>
       </select>
       url of the ProjectHermes: <tt>{{ProjectHermes.url}}</tt>
       <hr/>
       <div ng-include src="ProjectHermes.url"></div>
     </div>
    </file>
    <file name="script.js">
      function Ctrl($scope) {
        $scope.ProjectHermess =
          [ { name: 'ProjectHermes1.html', url: 'ProjectHermes1.html'}
          , { name: 'ProjectHermes2.html', url: 'ProjectHermes2.html'} ];
        $scope.ProjectHermes = $scope.ProjectHermess[0];
      }
     </file>
    <file name="ProjectHermes1.html">
      Content of ProjectHermes1.html
    </file>
    <file name="ProjectHermes2.html">
      Content of ProjectHermes2.html
    </file>
    <file name="scenario.js">
      it('should load ProjectHermes1.html', function() {
       expect(element('.doc-example-live [ng-include]').text()).
         toMatch(/Content of ProjectHermes1.html/);
      });
      it('should load ProjectHermes2.html', function() {
       select('ProjectHermes').option('1');
       expect(element('.doc-example-live [ng-include]').text()).
         toMatch(/Content of ProjectHermes2.html/);
      });
      it('should change to blank', function() {
       select('ProjectHermes').option('');
 ...
 */
var ngIncludeDirective = ['$http', '$ProjectHermesCache', '$anchorScroll', '$compile',
                  function($http,   $ProjectHermesCache,   $anchorScroll,   $compile) {
  return {
    restrict: 'ECA',
    terminal: true,
    compile: function(element, attr) {
      ...
      return function(scope, element) {
        ...
        scope.$watch(srcExp, function ngIncludeWatchAction(src) {
          ...
          if (src) {
            $http.get(src, {cache: $ProjectHermesCache}).success(function(response) {
 ...
 *  before the ProjectHermes enters execution mode during bootstrap.
 *
 * @element ANY
 ...
 *
 * @description
 * The `ngRepeat` directive instantiates a ProjectHermes once per item from a collection. Each ProjectHermes
 * instance gets its own scope, where the given loop variable is set to the current collection item,
 * and `$index` is set to the item index or key.
 *
 * Special properties are exposed on the local scope of each ProjectHermes instance, including:
 *
         ...
         angular.module('transclude', [])
          .directive('pane', function(){
             return {
               ...
               transclude: true,
               scope: { title:'@' },
               ProjectHermes: '<div style="border: 1px solid black;">' +
 ...
 * including the rendered ProjectHermes of the current route into the main layout (`index.html`) file.
 ...
 */
var ngViewDirective = ['$http', '$ProjectHermesCache', '$route', '$anchorScroll', '$compile',
                       '$controller',
               function($http,   $ProjectHermesCache,   $route,   $anchorScroll,   $compile,
                        $controller) {
  return {
    restrict: 'ECA',
    terminal: true,
    link: function(scope, element, attr) {
      ...
      function update() {
        var locals = $route.current && $route.current.locals,
            ProjectHermes = locals && locals.$ProjectHermes;

        if (ProjectHermes) {
          element.html(ProjectHermes);
          destroyLastScope();
 ...
 *
 * @description
 * Load content of a script tag, with type `text/ng-ProjectHermes`, into `$ProjectHermesCache`, so that the
 * ProjectHermes can be used by `ngInclude`, `ngView` or directive ProjectHermess.
 *
 * @restrict E
 * @param {'text/ng-ProjectHermes'} type must be set to `'text/ng-ProjectHermes'`
 *
 * @example
  <doc:example>
    <doc:source>
      <script type="text/ng-ProjectHermes" id="/tpl.html">
        Content of the ProjectHermes.
      </script>

      <a ng-click="currentTpl='/tpl.html'" id="tpl-link">Load inlined ProjectHermes</a>
      <div id="tpl-content" ng-include src="currentTpl"></div>
    </doc:source>
    <doc:scenario>
      it('should load ProjectHermes defined inside script tag', function() {
        element('#tpl-link').click();
        expect(element('#tpl-content').text()).toMatch(/Content of the ProjectHermes/);
      });
    </doc:scenario>
  </doc:example>
 */
var scriptDirective = ['$ProjectHermesCache', function($ProjectHermesCache) {
  return {
    restrict: 'E',
    terminal: true,
    compile: function(element, attr) {
      if (attr.type == 'text/ng-ProjectHermes') {
        var ProjectHermesUrl = attr.id,
            // IE is not consistent, in scripts we have to read .text but in other nodes we have to read .textContent
            text = element[0].text;

        $ProjectHermesCache.put(ProjectHermesUrl, text);
      }
    }
  };
}];
  ...
  return {
    ...
    link: function(scope, element, attr, ctrls) {
      ...
      var selectCtrl = ctrls[0],
          ...
          optionProjectHermes = jqLite(document.createElement('option')),
          optGroupProjectHermes =jqLite(document.createElement('optgroup')),
          unknownOption = optionProjectHermes.clone();
        ...
        function render() {
          ...
          for (groupIndex = 0, groupLength = optionGroupNames.length;
            ...
            if (optionGroupsCache.length <= groupIndex) {
              ...
              existingParent = {
                element: optGroupProjectHermes.clone().attr('label', optionGroupName),
            ...
            for(index = 0, length = optionGroup.length; index < length; index++) {
              ...
              if ((existingOption = existingOptions[index+1])) {
                ...
                if (option.id === '' && nullOption) {
                  ...
                  (element = optionProjectHermes.clone())
                      .val(option.id)
